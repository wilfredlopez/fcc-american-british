...
const britishOnlyMore = {};
for (let key in americanOnly) {
  britishOnlyMore[americanOnly[key]] = key;
}

const americanOnlyMore = {};
for (let key in britishOnly) {
  americanOnlyMore[britishOnly[key]] = key;
}
...
const setupBRData = [
  ...
  ...Object.entries(britishOnlyMore),
...
const setupENData = [
  ...
  ...Object.entries(americanOnlyMore),
...
const BritishSet = new Map(setupENData);
BritishSet.set(".", ":");
const AmericanSet = new Map(setupBRData);
AmericanSet.set(":", ".");
const DEFAULT_FRASE = "Everything looks good to me!";

export function toBritish(eng) {
  return Translate(eng, BritishSet, wordToBritish);
}

function wordToBritish(w) {
  return WordTranslate(w, BritishSet);
}

function WordTranslate(w, WordSet) {
  if (w.trim() === "") {
    return w;
  }
  const isLower = w.slice(0, 1) === w.slice(0, 1).toLowerCase();
  const hasNonWord = w.match(/\W/gim);

  let trans = WordSet.get(w.replace(/\W/, "").toLowerCase()) || w;
  if (!isLower) {
    trans = trans.slice(0, 1).toUpperCase() + trans.slice(1);
  }
  if (hasNonWord && trans.toLowerCase() !== w.toLowerCase()) {
    trans += hasNonWord.join(/\W/gim);
  }
  return trans;
}

export function toAmerican(bri) {
  return Translate(bri, AmericanSet, wordToAmerican);
}
function wordToAmerican(w) {
  return WordTranslate(w, AmericanSet);
}

function isTime(frase) {
  const part1 = isNaN(parseInt(frase.slice(0, 2)));
  const part2 = isNaN(parseInt(frase.slice(3, 5)));
  if (part1 || part2) {
    return false;
  }
  return true;
...
function Translate(langFrase, SetContainer, wordMapper) {
  const returnType = {
    translation: "",
    match: "",
    unmatch: "",
  };
  if (typeof langFrase !== "string" || langFrase === "") {
    return returnType;
  }

  let index = langFrase.indexOf(".");
  index = index !== -1 ? index : langFrase.indexOf(":");
  let separator = SetContainer.get(langFrase[index]);
  if (isTime(langFrase)) {
    if (separator) {
      returnType.unmatch = DEFAULT_FRASE;
      return returnType;
    }
    separator = ":";
    const m = langFrase.slice(0, index) + separator +
      langFrase.slice(index + 1, 5);
    const u = langFrase.slice(5);
    returnType.match = m;
    let split = splitMap(u, wordMapper);
    if (u === split) {
      returnType.unmatch = split;
    } else {
      returnType.match += split;
    }
    return returnType;
  }

  let translation = SetContainer.get(langFrase.toLowerCase());

  if (translation) {
    returnType.translation = translation;
    returnType.match = translation;
    return returnType;
  }

  translation = "";
  const foundMatches = [];
  for (let i = 0; i < langFrase.length; i++) {
    translation += langFrase[i];
    if (SetContainer.has(translation.toLowerCase())) {
      const isLower =
        translation.slice(0, 1) === translation.slice(0, 1).toLowerCase();
      translation = SetContainer.get(translation.toLowerCase());
      if (!isLower) {
        translation = translation.slice(0, 1).toUpperCase() +
          translation.slice(1);
      }
      foundMatches.push(translation);
    }
  }

  const match = foundMatches.join();

  translation = translation.replace(match, "");

  translation = splitMap(translation, wordMapper);
  const originalTranslation = translation;
  translation = match + translation;
  if (langFrase.toLowerCase() === translation.toLowerCase()) {
    returnType.unmatch = DEFAULT_FRASE;
    return returnType;
  }
  returnType.translation = translation.trim() === ""
    ? DEFAULT_FRASE
    : translation;
  returnType.match = match;
  returnType.unmatch = translation.trim() === ""
    ? DEFAULT_FRASE
    : originalTranslation;
  return returnType;
}

function splitMap(str, wordMapper) {
  return str.split(" ").map((w) => wordMapper(w))
    .join(" ");
}
...
function findMatch(letters, dict) {
  // Clone the array for manipulation
  var curLetters = letters.slice(0), word = "";
...
function searhIfMatch(word, dict) {
  return word.split(splitRegex).map((w) => {
    return w.split(splitRegex).map((v) => findMatch(v.split(""), dict)).filter(
      (v) => v,
    );
  }).filter((v) => v.length);
}
function searhIfWord(word, dict) {
  return word.split(splitRegex).map((w) => {
    return w.split(splitRegex).map((v) => findWord(v.split(""), dict)).filter(
      (v) => v,
    );
  }).filter((v) => v.length);
}

function tranlate(SEARCH_VALUE, dict) {
  let result = SEARCH_VALUE;
  const matches = searhIfMatch(SEARCH_VALUE, dict);
  const words = searhIfWord(SEARCH_VALUE, dict);
  for (let m of matches) {
    result = result.replace(m, "");
  }
  for (let i = 0; i < matches.length; i++) {
    result = result.replace(words[i], matches[i]);
  }
...
function translateToEnglish(str) {
  return tranlate(str, allEnligsh);
}

function translateToBritish(str) {
  return tranlate(str, allBritish);
}

const SEARCH_VALUE = "I spent the bank holiday at the funfair.";
const output = "I spent the public holiday at the carnival.";

// console.log(translateToBritish(SEARCH_VALUE));
// console.log(translateToEnglish(SEARCH_VALUE));
